( function( $ ){
    $( document ).ready( function(){
      $( '.real-estate-property-btn-get-started' ).on( 'click', function( e ) {
          e.preventDefault();
          $( this ).html( 'Processing.. Please wait' ).addClass( 'updating-message' );
          $.post( real_estate_property_ajax_object.ajax_url, { 'action' : 'install_act_plugin' }, function( response ){
        ...
        $.ajax( ajaxurl,
          {
            type: 'POST',
            data: {
              action: 'real_estate_property_dismissed_notice_handler',
              type: type,
            }
          } );
      } );
